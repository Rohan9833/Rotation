...
async function broadcastPresence(io, room) {
  ...
  io.to(room).emit("presence", { displays, scanners: sockets.length - displays });
}
...
export function setupSocket(io) {
  ...
  io.on("connection", async (socket) => {
    ...
    // a (re)connecting display catches up with the latest scan
    if (role === "display") {
      const user = await User.findById(userId).select("currentSequence");
      socket.emit("state", { sequence: user?.currentSequence ?? null });
    }

    broadcastPresence(io, room);
    socket.on("disconnect", () => broadcastPresence(io, room));
  });
}
